  ...
  group('Aggregation gateway', () {
    ...
    test('client reads from server A, server B exposes same data', () async {
      ...
      serverB!.addVariableNode(
        boolNodeId,
        DynamicValue(
            value: boolData.value, typeId: boolData.typeId, name: "the.bool"),
      );
      await serverB!.writeAttribute(
        boolNodeId,
        AttributeId.UA_ATTRIBUTEID_DESCRIPTION,
        boolData.description!,
      );
      ...
      serverB!.addVariableNode(
        intNodeId,
        DynamicValue(
            value: intData.value, typeId: intData.typeId, name: "the.int"),
      );
      ...
      expect(attrFromB[boolNodeId]!.description!.value, "A boolean variable",
    ...
    test('client monitors server A, pushes changes to server B', () async {
      ...
      serverA!.addVariableNode(
        boolNodeId,
        DynamicValue(value: false, typeId: NodeId.boolean, name: "the.bool"),
      );
      serverB!.addVariableNode(
        boolNodeId,
        DynamicValue(value: false, typeId: NodeId.boolean, name: "the.bool"),
      ...
      final subscription = stream.listen((data) {
        ...
        serverB!.write(
            boolNodeId,
            DynamicValue(
                value: data.value, typeId: NodeId.boolean));
      ...
      await clientA!.write(
          boolNodeId, DynamicValue(value: true, typeId: NodeId.boolean));
      await Future.delayed(Duration(milliseconds: 200));
      await clientA!.write(
          boolNodeId, DynamicValue(value: false, typeId: NodeId.boolean));
      ...
      expect(result.value, false,
          reason: 'Server B should have the latest value from Server A');
    });

    test('aggregation with all basic types (bool, int, double, string)',
        () async {
      ...
      expect(doubleFromB.value, closeTo(3.14, 0.001),
          reason: 'Double value should match');

      final stringFromB = await clientB!.read(stringNodeId);
      expect(stringFromB.value, "Hello World!",
          reason: 'String value should match');
    });
    ...
    test('browse replication from server A to server B', () async {
      ...
      expect(serverBBrowseAfter.length,
