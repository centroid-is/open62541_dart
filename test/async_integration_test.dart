...
Future<ClientApi> createClient(String clientType) async {
  switch (clientType) {
    case 'isolate':
      return await ClientIsolate.create(
...
void main() async {
  ...
  group('Integration tests', () {
    for (final clientType in ['isolate', 'direct']) {
      group('Client type: $clientType', () {
        setUp(() async {
          server = setupServer(lib, port);
          client = await createClient(clientType);
        ...
        test('Basic subscription', () async {
          ...
          final subscription =
              await client!.subscriptionCreate(requestedPublishingInterval: Duration(milliseconds: 10));
