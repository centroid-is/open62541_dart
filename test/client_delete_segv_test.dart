...
void main() {
  ...
  test(
    'delete() with a single active monitored-item stream: no crash, no stream errors',
    () async {
      await setup();
  ...
  test(
    'delete() with MULTIPLE active monitored-item streams: no crash, no stream errors',
    () async {
      await setup();
      ...
      expect(
        ...
        isEmpty,
        reason:
            'delete() must tear all streams down cleanly before freeing the client, so no error should be delivered',
      );
